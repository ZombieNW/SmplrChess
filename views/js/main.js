...
function checkStatus(color) {
  //flip names, idk why this bug happens
  var textcolor;
  if(color == "black"){
    textcolor = "White";
  }
  ...
  if (game.in_checkmate()) {
    $('#status').html(`<b>${textcolor}</b> loses, checkmate.`);
  } else if (game.insufficient_material()) {
    $('#status').html(`<b>Draw!</b> (Insufficient Material)`);
  } else if (game.in_threefold_repetition()) {
    $('#status').html(`<b>Draw!</b> (Threefold Repetition)`);
  } else if (game.in_stalemate()) {
    $('#status').html(`<b>Draw!</b> (Stalemate)`);
  } else if (game.in_draw()) {
    $('#status').html(`<b>Draw!</b> (50-move Rule)`);
  } else if (game.in_check()) {
    $('#status').html(`${textcolor}</b> is now in <b>check</b>`);
    return false;
  } else {
    $('#status').html(`<br>`);
    return false;
